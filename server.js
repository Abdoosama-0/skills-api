...
// Middleware to log every incoming request with timestamp, method, and path
...
// Get all skills (with query support)
app.get('/api/skills', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error reading skills data', error: error.message });
  }
});
...
app.get('/api/skills/category/:category', async (req, res) => {
  try {
    ...
    const filteredSkills = skills.filter(
      skill => skill.category.toLowerCase() === req.params.category.toLowerCase()
    );
...
// http://localhost:3001/  → API is running

// http://localhost:3001/api/skills 

// http://localhost:3001/api/skills/1 

// http://localhost:3001/api/skills?category=Frontend 

// http://localhost:3001/api/skills?proficiency=Expert&sort=name 